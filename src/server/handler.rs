...
use crate::semantic::capabilities::{CapabilitySet, Capability};
...
impl RpcHandler for ConnectionRpcHandler {
    fn call_remote(&self, function: &str, args: Vec<Value>) -> crate::runtime::error::RuntimeResult<Value> {
        ...
        if let Err(e) = self.message_tx.send(message) {
            return Err(crate::runtime::error::RuntimeError::General(
                format!("Failed to send remote call: {}", e)
            ));
        }
        ...
        match result {
            Ok(Ok(Ok(json_value))) => {
                json_to_runtime_value(&json_value).map_err(|e| {
                    crate::runtime::error::RuntimeError::General(format!("Response conversion error: {}", e))
                })
            }
            Ok(Ok(Err(err_msg))) => {
                Err(crate::runtime::error::RuntimeError::General(err_msg))
            }
            Ok(Err(_)) => {
                Err(crate::runtime::error::RuntimeError::General(
    ...
    fn call_method(&self, object: &Value, method: &str, args: Vec<Value>) -> crate::runtime::error::RuntimeResult<Value> {
        ...
        if let Err(e) = self.message_tx.send(message) {
            return Err(crate::runtime::error::RuntimeError::General(
                format!("Failed to send remote method call: {}", e)
            ));
        }
...
pub async fn handle_message(
    ...
    match message {
        Message::Request(req) => {
            Some(Message::Response(handle_request(req, manager).await))
        }
        
        Message::ExecutionStart(exec_start) => {
            Some(Message::ExecutionResult(handle_execution_start(exec_start, manager, pending_calls, message_tx).await))
        }
        
        Message::CodeRegister(code_reg) => {
            Some(Message::CodeRegisterResponse(handle_code_register(code_reg, manager).await))
        }
        
        Message::CachedExecutionStart(cached_exec) => {
            Some(Message::ExecutionResult(handle_cached_execution_start(cached_exec, manager, pending_calls, message_tx).await))
        }
        
        Message::RegisterFunction(reg_fn) => {
            Some(Message::RegisterResponse(handle_register_function(reg_fn, manager).await))
        }
        
        Message::RegisterMethod(reg_method) => {
        ...
        Message::RemoteFunctionResult(result) => {
            println!("DEBUG: Handling RemoteFunctionResult for response_id {}", result.response_id);
...
async fn handle_cached_execution_start(
    ...
    let exec_result = tokio::task::spawn_blocking(move || {
        crate::runtime::execute(&cached_ast, &mut runtime_ctx)
    }).await;
...
async fn handle_register_method(
    ...
    {
        let mut reg = registry.write().await;
        reg.allow_method(&reg_method.context_id, &reg_method.object_id, &reg_method.method_name);
    }
