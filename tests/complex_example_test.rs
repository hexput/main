...
fn test_05_complex_example() {
    ...
    // Parse the script
    let ast = hexput::language::parse(&source)
    ...
    // Execute the script
    let result = hexput::runtime::execute(&ast, &mut context)
        .expect("Execution failed");
    
    println!("Execution successful!");
    let json_value = value_to_json(&result);
    println!("Result: {}", serde_json::to_string_pretty(&json_value).unwrap());
    ...
    assert_eq!(results_obj.len(), 2, "Should have processed 2 items (json, csv)");
    
    // Check json result
    let json_result = results_obj.get("json").expect("json result should exist");
    let json_obj = json_result.as_object().expect("JSON result should be object");
    assert!(json_obj.contains_key("value"), "Should have value field");
    assert!(json_obj.contains_key("score"), "Should have score field");
    assert!(json_obj.contains_key("metadata"), "JSON should have metadata");
    assert_eq!(json_obj.get("score").and_then(|v| v.as_f64()), Some(20.0), "JSON score should be 20");
    ...
    assert_eq!(csv_obj.get("score").and_then(|v| v.as_f64()), Some(15.0), "CSV score should be 15");
...
fn test_05_complex_with_bytecode() {
    ...
    let bincode_data = bincode::serialize(&ast)
        .expect("Failed to serialize AST");
    
    let bytecode = base64::Engine::encode(&base64::engine::general_purpose::STANDARD, &bincode_data);
    ...
    let deserialized_ast: hexput::language::ast::Ast = bincode::deserialize(&decoded)
