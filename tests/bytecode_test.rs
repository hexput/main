...
use hexput::rpc::protocol::{BytecodeExecutionStart, CompileBytecode, Message, ResponseResult};
use serde_json::json;
...
async fn test_compile_simple_arithmetic() {
    ...
    if let Message::CompileBytecodeResponse(resp) = response {
        assert!(
            !resp.bytecode.starts_with("error: "),
        ...
        assert!(
            base64::Engine::decode(&base64::engine::general_purpose::STANDARD, &resp.bytecode)
                .is_ok(),
            "Bytecode should be valid base64"
        );
    } else {
...
async fn test_compile_with_syntax_error() {
    ...
    if let Message::CompileBytecodeResponse(resp) = response {
        assert!(
            resp.bytecode.starts_with("error: "),
            "Should return error for syntax error"
        );
    } else {
        panic!("Expected CompileBytecodeResponse, got {:?}", response);
    }
...
async fn test_compile_and_execute_simple() {
    ...
    let compile_response =
        send_and_receive(&mut write, &mut read, compile_msg, compile_request_id).await;
...
async fn test_execute_same_bytecode_multiple_times() {
    ...
    for (i, expected) in [(10.0, 20.0), (15.0, 30.0), (7.5, 15.0)].iter() {
        ...
        let exec_response =
            send_and_receive(&mut write, &mut read, exec_msg, exec_request_id).await;
...
async fn test_execute_invalid_bytecode() {
    ...
    if let Message::ExecutionResult(result) = exec_response {
        match result.result {
            ResponseResult::Error { message } => {
                assert!(
                    message.contains("decode") || message.contains("deserialize"),
